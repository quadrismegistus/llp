...
def tabify(data):
	...
	if datatype(data)=='df':
		data=df2ld(data)
	elif not datatype(data).startswith('ld'):
		raise Exception('supply an ld or df')
	header=set()
	for dx in data: header|=set(dx.keys())
	header=sorted(list(header))

	ostr=['\t'.join(header)]
	for dx in data:
		ostr+=['\t'.join([unicode(dx[h]) for h in header])]
	return '\n'.join(ostr)
...
def writegen(fnfn,generator,header=None):
	...
	generator=generator() if not isinstance(generator,types.GeneratorType) else generator
